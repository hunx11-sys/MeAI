...
ITCK = {'x': ['x_ct']}
# 약관이 수가코드를 낱낱이 열거한 검사 항목 — 같은 '조직검사·내시경'이라도 특약마다 해당 여부가 다르다(v8.61).
#   · 암 특정생검조직병리검사(급여) : 암 통합치료비(기본형) 제5조 ⑦ 수가코드 31개(침생검(심부)·절개생검(개흉·개복)·전립선·유방·갑상선 등 장기별 생검)
#       → 내시경 겸자생검·기관지내시경 생검·림프절 생검·피부 생검은 목록에 없다. 「암 내시경검사(급여)」는 제4절 내시경 전체라 위·대장내시경도 지급.
#   · 특정내시경(급여) : 암전후관련 특정질환 및 양성신생물 통합치료비 제9조 수가코드 열거(흉강경·종격동·ERCP·담도경·풍선소장내시경·후복강경·요관경·신내시경·캡슐내시경)
#       → 위·대장내시경은 없다.
#   · 특정생검(급여)   : 같은 특약 제8조 — 침생검·절개생검 수가코드만, 「수술 중 발생한 절제생검(excisional biopsy)」 제외.
#   통합치료비 엔진(engine.calc_rider)이 no:{특약종류:[키]} 를 보고 그 항목을 0원(why 'def')으로 두고 nor 의 사유를 로그에 남긴다.
_NOR_BIO_ENDO = '내시경으로 떼어낸 조직검사는 「암 특정생검조직병리검사(급여)」 수가코드 31개(침생검·절개생검·장기별 생검)에 없어요. 내시경검사 자체는 지급돼요'
NO_BIO_ENDO = {'no': {'CB': ['x_bio']}, 'nor': {'x_bio': _NOR_BIO_ENDO}}
NO_BIO_BRONCH = {'no': {'CB': ['x_bio']}, 'nor': {'x_bio': '기관지내시경으로 떼어낸 조직검사는 수가코드 목록에 없어요. CT 유도 경피적 침생검(침생검(심부)-장기 C8513)으로 받았다면 해당해요'}}
_NOR_PR_ENDO = '특정내시경(급여)은 약관 제9조에 열거된 수가코드(흉강경·담췌관·소장·후복강경·요관경·신내시경 등)만 해당해요. 위·대장내시경은 없어요'
NO_PR_ENDO = {'no': {'PR': ['x_endo']}, 'nor': {'x_endo': _NOR_PR_ENDO}}
NO_PR_ENDO_BIO = {'no': {'PR': ['x_endo', 'x_bio']}, 'nor': {'x_endo': _NOR_PR_ENDO,
                  'x_bio': '특정생검(급여)은 침생검·절개생검 수가코드만 해당하고, 약관 제8조가 「수술 중 발생한 절제생검(excisional biopsy)」을 제외해요. 내시경 겸자생검도 목록에 없어요'}}
# ── 암 ─────────────────────────────────────────────
def cancer_cells():
...
def page_cv():
    ...
    for k, kcd, dx, grp, cols in [
        ('cv', 'I63', 'brain', G, [('혈전용해치료만', '혈전용해제(tPA) 주사 · 수술 없음', [['시술', '혈전용해', '', ['thromb']]], None, None, None),
                                   ...
                                   ('뇌혈관 스텐트 삽입술', '경피적 뇌혈관 수술(비관혈)', [['시술', '스텐트 삽입', '', ['surg']]], '88-1', None, 'B026'),
        ...
        for t, s2, itc, j, ac, j7 in cols:
            ico = {'혈전용해치료만':'drop2','혈전용해 + 혈전제거술':'drop2','뇌혈관 스텐트 삽입술':'stent','개두 수술':'neuro_surgery','관상동맥 스텐트':'stent','심장 개흉수술':'heart_organ'}.get(t,'stethoscope')
...
FLOW = [
 dict(k='ca', ic='lungs', t='폐암', sub='진단 → 흉강경 폐절제 → 표적·면역(키트루다) 항암 → 입원', kcd='C34',
      steps=[('검사', 'microscope', '흉부 CT · PET · 조직검사', [['검사', '흉부 CT · PET · 조직검사', '', ['x_ct', 'x_pet', 'x_bio'], NO_BIO_BRONCH]], dict(dx='cancer', hc=['CB003', 'CB004'])),   # NGS 유전자패널(고형암 Level I·II) 수가코드
 ...
 dict(k='ca', ic='stomach', t='위암', sub='진단 → 복강경 위아전절제 → 보조 항암 → 입원', kcd='C16', extra=True,
      steps=[('검사', 'microscope', '위내시경 · 조직검사 · 복부 CT', [['검사', '위내시경 · 조직검사 · CT', '', ['x_endo', 'x_bio', 'x_ct'], NO_BIO_ENDO]], dict(dx='cancer')),
 ...
 dict(k='ca', ic='prostate_cancer', t='전립선암', sub='진단 → 로봇 근치적 전립선절제 → 방사선 → 호르몬 치료', kcd='C61', extra=True, sex='M',
      steps=[('검사', 'microscope', 'PSA 혈액검사 · 전립선 MRI · 조직검사', [['검사', 'PSA · MRI · 조직검사', '', ['x_mri', 'x_bio']]], dict(dx='cancer')),
             ('수술', 'surgical_sterilization', '로봇 보조 근치적 전립선절제술', [['수술', '로봇 보조 근치적 전립선절제술', '', ['surg', 'robot'], {'nc': 1}]], dict(surg='C1', surg7='M021', hosp='상급종합', anes=1)),   # 문구대로 로봇(비급여) 행위 표시 — cancer_cells 의 로봇수술 셀과 같은 태그(v8.61)
 ...
 dict(k='ca', ic='colon', t='대장암', sub='진단 → 복강경 결장절제 → 보조 항암 → 입원', kcd='C18', extra=True, neutral=True,
      steps=[('검사', 'microscope', '대장내시경 · 조직검사 · 복부 CT', [['검사', '대장내시경 · 조직검사 · CT', '', ['x_endo', 'x_bio', 'x_ct'], NO_BIO_ENDO]], dict(dx='cancer')),
...
# ── 다빈도 질환 ────────────────────────────────────
FREQ = [
 # 갑상선결절 : 고주파 절제술(RFA)은 1-5종 수술분류표 어느 항목에도 없어(88번 경피적 수술의 부위 목록에 갑상선이 없음)
 #   종을 정할 수 없다 — 분류표에 있는 갑상선 부분절제술(57 · 3종 / 1-7종 K076 · 2종)로 예시한다(v8.55)
 # 전신마취(anes=1)는 갑상선·복강경·편도절제처럼 전신마취로 하는 수술에만 둔다. 마취시간(anes_h)은 적지 않아 기본(급여) 항목만 계산된다(v8.61)
 ('갑상선결절', 'D34', 'thyroid', '갑상선 부분절제술', '급여 · 3종 수술', dict(surg='57', surg7='K076', grp=['다빈도62대질병'], anes=1), [['수술', '갑상선 부분절제', '', ['surg'], {'j': 3}]]),
 ...
 ('자궁근종', 'D25', 'cervical_cancer', '복강경 자궁근종 절제술', '급여 · 2종 수술', dict(surg='52', surg7='N031', grp=['다빈도62대질병'], anes=1), [['수술', '복강경 근종 절제', '', ['surg'], {'j': 2}]]),
 ('대장 용종', 'D12', 'colon', '내시경 용종절제술', '급여 · 1종 수술', dict(surg='88-2', surg7='G523', grp=['다빈도62대질병'], five_major=True, hc=['Q7701', 'Q7702', 'Q7703', 'QX706']), [['진단', '대장내시경', '', ['x_endo'], NO_PR_ENDO], ['수술', '용종 절제', '', ['surg'], {'j': 1}]]),
 ('담석증', 'K80.0', 'gallbladder', '복강경 담낭절제술', '급여 · 2종 수술', dict(surg='36', surg7='H107', grp=['다빈도62대질병'], days=5, room='2-3인실', hosp='모든', anes=1), [['진단', '복부 CT', '', ['x_ct']], ['수술', '복강경 담낭 절제', '', ['surg'], {'j': 2}]]),
 ('백내장', 'H25.9', 'body', '수정체 유화술 + 인공수정체', '급여 · 1종 수술', dict(surg='71', surg7='C061', grp=['백내장']), [['수술', '수정체 유화술 + 인공수정체', '', ['surg'], {'j': 1}]]),
 ('디스크(추간판장애)', 'M51', 'body', '내시경 추간판 제거술', '급여 · 2종 수술', dict(surg='88-2', surg7='B174', grp=['다빈도62대질병']), [['수술', '내시경 추간판 수술', '', ['surg'], {'j': 2}]]),
 ('치핵', 'K64', 'intestine', '치핵절제술', '급여 · 1종 수술', dict(surg='44', surg7='G272', grp=['치핵']), [['수술', '치핵절제술', '', ['surg'], {'j': 1}]]),
 ('급성 충수염(맹장염)', 'K35', 'intestine', '복강경 충수절제술', '급여 · 2종 수술', dict(surg='41', surg7='G212', grp=[], anes=1), [['진단', '복부 CT', '', ['x_ct']], ['수술', '복강경 충수 절제', '', ['surg'], {'j': 2}]]),
 ('편도염(만성·재발)', 'J35', 'body', '편도절제술', '급여 · 1종 수술', dict(surg='17', surg7='D162', grp=['다빈도62대질병'], anes=1), [['수술', '편도 절제', '', ['surg'], {'j': 1}]]),
]
def freq_rows():
    out = ''
...
ITC_ALT = {
 'ms': [dict(k='ms', t='외상성 뇌출혈', kcd='S06.5',
        ...
        dict(k='ms', t='죽상경화증(하지동맥 폐쇄)', kcd='I70.2',
             steps=[('검사', 'xray', '하지 혈관 CT · 초음파', [['검사', '혈관 CT', '', ['x_ct']]], dict(grp=[])),
                    ('수술', 'knife', '말초동맥 혈관 관혈수술', [['수술', '혈관 관혈수술', '', ['surg'], {'j': 3}]], dict(surg='22', hosp='종합')),   # 1-5종 22 혈관관혈수술(3종). 경피적 스텐트는 1-5종 88번 부위 목록에 말초혈관이 없어 종을 정할 수 없다(v8.55)
 ...
 'pre': [dict(k='pr', t='담석증(급성 담낭염)', kcd='K80.0',
              steps=[('검사', 'xray', '복부 CT · 초음파', [['검사', '복부 CT', '', ['x_ct']]], dict(grp=[])),
                     ('수술', 'knife', '복강경 담낭절제술', [['수술', '복강경 담낭절제', '', ['surg'], {'j': 2}]], dict(surg='36', surg7='H107', hosp='종합')),   # 1-5종 36 의 담석증(K80) 예외 = 2종(v8.55)
                     ('입원', 'hospital', '종합병원 2-3인실 5일', [], dict(hosp='종합', room='2-3인실', days=5))]),
         dict(k='pr', t='궤양성 대장염', kcd='K51',
              steps=[('검사', 'microscope', '대장내시경 · 조직검사', [['검사', '대장내시경 · 조직검사', '', ['x_endo', 'x_bio'], NO_PR_ENDO_BIO]], dict(grp=[])),
...
LSNOTE = ('<div class="mnote">※ 통합생활지원비는 <b>산정특례 등록·치료를 받은 달</b>마다 항목별로 지급되고, 한 달 합계는 가입금액까지예요. '
          ...
          '전신마취는 마취시간을 적어 둔 사례(교통사고 개두술 6시간 · 가정값)만 4시간·6시간 항목을 더했고, 나머지 수술은 <b>기본(급여)</b> 금액으로만 계산했어요 — 실제 마취시간이 4시간·6시간 이상이면 더 커져요.</div>') if F['ls'] else ''
...
# 6쪽 : 상해·사고
# 두개내손상 개두술 : 아래 INJ_CASES(상해 통합치료비)가 이미 '전신마취 6시간이상' 항목을 넣고 있으므로 같은 가정(anes_h=6 · 가정값)을
# 이 사례 태그에도 둔다 — 종합병원 전신마취치료비 3종·통합생활지원비가 같은 쪽에서 다른 답을 내지 않게(v8.61)
INJ = [('교통사고 두개내손상', 'S06', 'wound', dict(cause='상해', surg='59', surg7='B122', hosp='종합', room='2-3인실', days=20, icu=3, grp=[], anes=1, anes_h=6),
       ...
       ('열린 상처 · 창상봉합술', 'T14', 'bandage_adhesive', dict(cause='상해', surg='창상봉합', surg7='X060', hosp='종합', room=None, days=0, grp=[]), []),   # 창상봉합은 1-5종 분류표에 없다 → 1-5종 담보는 계산 제외(로그) · 1-7종은 X060 기타 손상의 수술(v8.55)
...
INJ_CASES = [('교통사고 두개내손상', 'ambulance', ['x_ct', 'x_mri', 'icu', 'vent', 'anes6', 'rehab_in'], 5, 10, '검사 → 개두수술(5종) → 중환자실·인공호흡기 → 입원 재활 10일'),
             ...
             ('열린 상처 · 창상봉합술', 'bandage_adhesive', ['suture', 'aspir'], None, 0, '창상봉합술 · 흡인·절개'),
...
_LOG = S.issues_grouped()                    # 같은 사유의 로그는 담보 그룹 1줄로(담보목록에 전부 남김 · v8.61)
print('  [인식] 마스터 %(마스터)d · 부모연결 %(부모연결)d · 규칙만 %(규칙만)d · 계산제외 %(계산제외)d' % A['인식'])
for x in _LOG[:10]: print('  [%s] %s — %s' % (x['구분'], x['담보'][:34], x['사유']))
json.dump({'요약': A, '로그': _LOG}, open(os.path.splitext(sys.argv[2])[0] + '_audit.json', 'w', encoding='utf-8'),
