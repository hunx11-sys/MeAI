...
> proposal_smart v8.48 · AI추진파트 세일즈혁신TF
...
python -m playwright install chromium     # HTML → PDF 렌더 (최초 1회) — playwright 1.47 이상
...
| 메서드 | 경로 | 요청 | 응답 |
|---|---|---|---|
| GET | `/health` | — | `{"ok":true,"version":"v8.48","llm":false,"network":false}` |
| GET | `/` | — | 연동 데모 화면(HTML) |
...
### v8.48·v8.49 — 지면 검사 결과
| 키 | 뜻 |
|---|---|
| `audit.요약.지면검사` | 쪽별 자동 축소율·아래 여백, 그리고 글자가 틀 밖으로 나간 경우 `이탈`·`잘림` 항목 |
| `audit.요약.렌더러` | 실제로 찍은 크로미엄 판(예 `chromium/141.0.7390.37`). 현장에서 지면이 이상하다는 제보가 오면 생성 PC와 판을 대 볼 때 쓴다 |
| `layout_warn` (응답 최상위) | `이탈`·`잘림`이 하나라도 있을 때만 실림. 있으면 그 PDF는 검수 후 배포한다 |

`이탈`·`잘림`은 정상 동작에서는 나오지 않는다(테스트 설계 6건 모두 0건). 나온다면 글꼴이 다르게 깔렸거나
크로미엄 판이 크게 낮은 환경이므로, `렌더러` 값을 함께 보고 환경을 맞춘다.

렌더는 `playwright` 가 함께 들고 오는 크로미엄을 쓴다. **playwright 1.47 이상**(크로미엄 129 이상)을 설치한다.
