...
OFFMARK = 'return; /* 오프라인 배포판: 사용 통계 전송 안 함 */'
...
README = """메리츠 세일즈혁신 영업지원도구 6종 (오프라인 배포판)
만든 날짜: {date}

■ 사용법
1. 이 압축파일을 원하는 폴더에 풉니다. (파일들이 같은 폴더에 있어야 합니다)
...
- 인터넷 익스플로러(IE)에서는 열리지 않습니다. 크롬 또는 엣지를 쓰세요.
...
def build(outname, with_font):
    ...
    font_line = ('- 글꼴은 화면 파일 안에 넣어 두었습니다. (Pretendard, SIL Open Font License 1.1)\n'
                 if fonted else
                 '- 글꼴은 윈도우 기본 글꼴(맑은 고딕)로 나옵니다.\n')
    report = font_line
