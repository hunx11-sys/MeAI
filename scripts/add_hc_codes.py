...
m = re.search(r'(<script id="DATA" type="application/json">)(.*?)(</script>)', html, re.S); D = json.loads(m.group(2)); T = D['tables']
n = 0
for r in D['riders']:
    codes = []
...
io.open(TOOL, 'w', encoding='utf-8').write(html[:m.start(2)] + json.dumps(D, ensure_ascii=False, separators=(',', ':')) + html[m.end(2):])
